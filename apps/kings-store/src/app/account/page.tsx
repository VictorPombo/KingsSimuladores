...
export default async function AccountPage({ searchParams }: { searchParams: { order?: string } }) {
  ...
  return (
    <div style={{ background: '#06080F', minHeight: '100vh', paddingTop: '100px', color: '#fff' }}>
      <Container>
        <h1 className="desktop-only" style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '2rem', color: '#00e5ff' }}>Sua Garagem</h1>
        
        {/* Header Mobile Otimizado (Mobile Only) */}
        <div className="mobile-only" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
        ...
        <div className="account-grid">
          ...
          {/* Main Area: Orders */}
          <div>
            <h2 className="desktop-only" style={{ fontSize: '1.2rem', marginBottom: '1rem', color: '#a1a1aa' }}>Meus Pedidos</h2>
                ...
                <div key={order.id} style={{ 
                  ...
                  {order.tracking_code && (
                    <div style={{ background: 'rgba(0,0,0,0.5)', padding: '1rem', borderRadius: '0.5rem', border: '1px dashed rgba(255,255,255,0.1)' }}>
                      <div style={{ fontSize: '0.9rem', color: '#a1a1aa', marginBottom: '0.25rem' }}>Código de Rastreio</div>
          ...
          {/* Sidebar: Profile (Somente Desktop - No mobile vai stackar embaixo mas vamos manter escondido o Sair pois já botamos em cima) */}
          <div className="desktop-only" style={{ background: 'rgba(255,255,255,0.02)', padding: '1.5rem', borderRadius: '0.5rem', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <form action={logout}>
              <button type="submit" style={{ width: '100%', marginTop: '2rem', padding: '0.5rem', background: 'rgba(255,0,0,0.1)', color: '#ff4444', border: '1px solid rgba(255,0,0,0.2)', borderRadius: '0.25rem', cursor: 'pointer' }}>
                Sair
              </button>
            </form>
          </div>
