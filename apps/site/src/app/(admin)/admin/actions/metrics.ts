...
export async function getAdminMetrics() {
  ...
  const stockValue = stockData?.reduce((acc, p) => acc + (p.price * (p.stock || 0)), 0) || 0
  ...
  const totalVisitasMes = visitsData?.reduce((acc, day) => acc + Number(day.unique_visits), 0) || 0
  ...
  const recentOrders = orders?.filter(o => o.created_at >= thirtyDaysAgoIso) || []
  
  const abandonedCarts = recentOrders.filter(o => ['draft', 'pending', 'checkout_started'].includes(o.status)).length
  const completedRecentOrders = recentOrders.filter(o => ['paid', 'shipped', 'completed'].includes(o.status)).length
