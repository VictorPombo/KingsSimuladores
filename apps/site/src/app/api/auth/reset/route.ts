...
export async function POST(req: Request) {
  try {
    ...
    // 1. Gera o link de recuperação manualmente pelo Supabase Auth Admin API
    const { data, error } = await adminSupabase.auth.admin.generateLink({
      type: 'recovery',
      email: email,
    })

    // Se o erro for UserNotFound, podemos falhar silenciosamente ou retornar erro, mas como é reset, 
    // retornar sucesso evita vazamento de dados. Para fins de dev, vamos logar.
    if (error) {
    ...
    // 2. Monta o HTML do email com o link
    const htmlBody = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        ...
        <p class="footer">Se você não solicitou essa redefinição, apenas ignore este e-mail. Nenhuma mudança será feita na sua conta.<br>&copy; 2026 Kings Simuladores</p>
      </div>
    </body>
    </html>
    `

    // 3. Dispara o e-mail via Resend API direta (Bypass do bug interno do Supabase GoTrue)
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: \`Kings Simuladores <\${process.env.RESEND_FROM_KINGS || 'noreply@kingssimuladores.com.br'}>\`,
        to: [email],
    ...
    if (!resendRes.ok) {
      ...
      return NextResponse.json({ error: 'Falha no provedor de email.' }, { status: 500 })
    }
