...
async function loginAsAdmin(page: Page): Promise<void> {
  ...
  await page.getByPlaceholder(/admin@/i).fill(ADMIN_EMAIL)
...
test.describe('Admin — Login', () => {
  ...
  test('exibe mensagem de erro com credenciais inválidas', async ({ page }) => {
    ...
    await page.getByPlaceholder(/admin@/i).fill('invalido@exemplo.com')
    await page.getByPlaceholder(/••••••••/).fill('senhaerrada123')

    // Não usar networkidle aqui — Supabase mantém conexões longas que causam timeout no teardown
    await Promise.all([
      page.getByRole('button', { name: /entrar/i }).click(),
      page.waitForResponse(
        (res) => res.url().includes('supabase') || res.url().includes('/auth/'),
        { timeout: TIMEOUTS.medium }
      ).catch(() => {}), // ignora se não houver response capturável
    ])
...
test.describe('Admin — Notas Fiscais', () => {
  ...
  test('/admin/notas-fiscais carrega título e botão de sincronizar', async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}/admin/notas-fiscais`, { waitUntil: 'domcontentloaded' })
...
test.describe('Admin — Páginas Auxiliares', () => {
  ...
  test('/admin/config-gerais carrega sem crash', async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}/admin/config-gerais`, { waitUntil: 'domcontentloaded' })
  ...
  test('/admin/dados-loja carrega sem crash', async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}/admin/dados-loja`, { waitUntil: 'domcontentloaded' })
  ...
  test('/admin/produtos carrega sem crash', async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}/admin/produtos`, { waitUntil: 'domcontentloaded' })
...
test.describe('Admin — Smoke: sem erros de console em rotas críticas', () => {
  ...
  test('navega por todas as rotas críticas e verifica ausência de erros', async ({ page }) => {
    ...
    for (const rota of rotasCriticas) {
      ...
      await page.goto(`${BASE_URL}${rota}`, { waitUntil: 'domcontentloaded', timeout: TIMEOUTS.long })
